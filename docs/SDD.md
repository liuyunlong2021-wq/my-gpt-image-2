...
- [ ] **D1**：是否批准整体方案（Prompt Hub + 免费 sync）？
- [ ] **D2**：是否接受数据规模 **~1,500–2,000** 而非 3 万+（免费）？
- [ ] **D3**：实施顺序是否采用 A → B → C？
- [ ] **D4**：是否删除 `data.json`，完全迁至 `data/{model}.json`？
- [ ] **D5**：主站 CTA URL 是否加 UTM（如 `?utm_source=dazi.studio&utm_medium=prompt_modal`）？
- [ ] **D6**：`mockup.html` 是否保留在仓库作长期参考，或合并后删除？
